...
# ===========================================================================
# Permet une modification facile des constantes sans fouiller le code
...
# ===========================================================================
# Séparation claire state/metrics → plus facile à tester/mocker
...
# Déterminer si une adresse est IPv4 ou IPv6
is_ipv6() {
    local ip="$1"
    [[ "$ip" =~ : ]] && return 0 || return 1
}

# ipt6 — wrapper iptables IPv6 (gracieux si pas disponible)
ipt6() {
    ip6tables "$@" 2>/dev/null || true
}

# Ajouter règle port 853 (auto-détect IPv4/IPv6)
ipt_add_853() {
    local ip="$1"
    if is_ipv6 "$ip"; then
...
# Tue un processus gracieusement
kill_pid() {
    local pid="$1"
    [ -n "$pid" ] && kill "$pid" 2>/dev/null || true
}

# Attend un processus + timeout
wait_for_port() {
...
# Exécute une commande, log JSON si erreur
run_checked() {
    local desc="$1"
    shift
...
get_vpn_port_proto() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        VPN_PROTO=$(awk '/^proto /{print $2; exit}' "$CONFIG_FILE")
...
check_vpn_ip() {
    ...
    if [ -f /etc/privoxy/privoxy.config ]; then
        ...
        addr=$(awk '/^[[:space:]]*listen-address/{print $2; exit}' \
            /etc/privoxy/privoxy.config || echo "")
...
_setup_iptables_output_dns() {
    # DNS local
    iptables -A OUTPUT -p udp -d 127.0.0.1 --dport "$PORT_DNS" -j ACCEPT
    iptables -A OUTPUT -p tcp -d 127.0.0.1 --dport "$PORT_DNS" -j ACCEPT
    iptables -A OUTPUT -p udp -d 127.0.0.1 --dport "$PORT_UNBOUND" -j ACCEPT
    iptables -A OUTPUT -p tcp -d 127.0.0.1 --dport "$PORT_UNBOUND" -j ACCEPT

    # Docker DNS interne
    grep -Fq "127.0.0.11" "$RESOLV_CONF" 2>/dev/null && {
        iptables -A OUTPUT -d 127.0.0.11 -j ACCEPT
        iptables -A OUTPUT -p udp -d 127.0.0.11 --dport "$PORT_DNS" -j ACCEPT
        iptables -A OUTPUT -p tcp -d 127.0.0.11 --dport "$PORT_DNS" -j ACCEPT
    }
}
...
_setup_iptables_output_dot() {
    ...
    # Bloquer tout DNS 53 externe
    iptables -A OUTPUT -p udp ! -d 127.0.0.0/8 --dport "$PORT_DNS" -j DROP
    iptables -A OUTPUT -p tcp ! -d 127.0.0.0/8 --dport "$PORT_DNS" -j DROP
}

_setup_iptables_output_standard_dns() {
    for dns in "${DNS_SERVER_1:-}" "${DNS_SERVER_2:-}"; do
        [[ "$dns" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] || continue
        iptables -A OUTPUT -p udp -d "$dns" --dport "$PORT_DNS" -j ACCEPT
        iptables -A OUTPUT -p tcp -d "$dns" --dport "$PORT_DNS" -j ACCEPT
    done
    while read -r dns; do
        [[ "$dns" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] || continue
        iptables -A OUTPUT -p udp -d "$dns" --dport "$PORT_DNS" -j ACCEPT
...
setup_iptables() {
    ...
    docker_network="$(ip -o addr show dev eth0 2>/dev/null | awk '$3=="inet"{print $4}' || echo "")"
    ...
    iptables -A OUTPUT -p tcp -d 127.0.0.1 --dport "$PORT_METRICS" -j ACCEPT
...
# IPv6 → même pattern, fonction compacte
setup_ip6tables() {
    command -v ip6tables >/dev/null 2>&1 || {
        log_json WARN "setup_ip6tables" "ip6tables not installed"; return 0
    }
    [ -f /proc/net/if_inet6 ] || {
        log_json WARN "setup_ip6tables" "IPv6 not available"; return 0
    }

    local docker6_network
    docker6_network="$(ip -o addr show dev eth0 2>/dev/null | awk '$3=="inet6"{print $4; exit}' || echo "")"
    ...
    ipt6 -A OUTPUT -p tcp -d ::1 --dport "$PORT_METRICS" -j ACCEPT
    ipt6 -A OUTPUT -p udp -d ::1 --dport "$PORT_DNS" -j ACCEPT
    ipt6 -A OUTPUT -p tcp -d ::1 --dport "$PORT_DNS" -j ACCEPT
    ipt6 -A OUTPUT -p udp -d ::1 --dport "$PORT_UNBOUND" -j ACCEPT
    ipt6 -A OUTPUT -p tcp -d ::1 --dport "$PORT_UNBOUND" -j ACCEPT
    ...
    if [ "${ENABLE_DOT:-false}" = "true" ]; then
        ...
        while read -r dns; do
            ...
            ipt6 -A OUTPUT -p udp -d "$dns" --dport "$PORT_DNS" -j ACCEPT
            ipt6 -A OUTPUT -p tcp -d "$dns" --dport "$PORT_DNS" -j ACCEPT
        done < <(get_dns_upstreams)
    fi
...
setup_return_routes() {
    ...
    [ -z "$iface" ] && {
        log_json WARN "setup_return_routes" "no default interface"; return 0
    }

    gw=$(ip -4 route show dev "$iface" 2>/dev/null | awk '/default/{print $3; exit}' || echo "")
    gw6=$(ip -6 route show dev "$iface" 2>/dev/null | awk '/default/{print $3; exit}' || echo "")
    ips=$(ip -4 addr show dev "$iface" 2>/dev/null | awk -F'[ /]+' '/inet /{print $3}' || echo "")
    ip6s=$(ip -6 addr show dev "$iface" 2>/dev/null | awk -F'[ /]+' '/inet6.*global/{print $3}' || echo "")
...
# Résoudre un hostname DoT — IPv4 uniquement
resolve_dot_host() {
    local host="$1" ip
    ip=$(getent ahostsv4 "$host" 2>/dev/null | awk '/STREAM/{print $1; exit}' || echo "")
    if [ -z "$ip" ]; then
        ip=$(nslookup "$host" 2>/dev/null | awk '/^Address: /{ if ($2 !~ /:/) {print $2; exit} }' || echo "")
    fi
    # Fallback : essayer DNS_SERVER_1/2
    if [ -z "$ip" ]; then
        for dns in "${DNS_SERVER_1:-}" "${DNS_SERVER_2:-}"; do
            [ -z "$dns" ] && continue
            ip=$(nslookup "$host" "$dns" 2>/dev/null | \
                awk '/^Address: /{ if ($2 !~ /:/) {print $2; exit} }' || echo "")
            [ -n "$ip" ] && {
...
configure_unbound() {
    ...
    command -v unbound >/dev/null 2>&1 || {
        log_json ERROR "configure_unbound" "unbound binary not found"; return 1
    }

    parse_dot_servers

    [ -s "$DOT_FORWARD_ADDRS_FILE" ] || {
        log_json ERROR "configure_unbound" "no valid DoT servers"; return 1
    }
...
_dot_refresh_loop() {
    ...
    while true; do
        ...
        for entry in $servers; do
            ...
            [ -z "$new_ip" ] && {
                log_json WARN "dot_refresh" "re-resolve failed" "host=${host}"; continue
            }
            [ "$new_ip" = "$old_ip" ] && {
                log_json INFO "dot_refresh" "IP unchanged" "host=${host}" "ip=${new_ip}"; continue
            }
            ...
            if configure_unbound; then
                local ub_pid
                ub_pid=$(pidof unbound | awk '{print $1}' || echo "")
                if [ -n "$ub_pid" ]; then
...
start_metrics() {
    ...
    command -v nc >/dev/null 2>&1 || {
        log_json WARN "start_metrics" "nc not available"; return 0
    }
...
drop_capabilities() {
    ...
    command -v python3 >/dev/null 2>&1 || {
        log_json WARN "drop_caps" "python3 not found"; return 0
    }
...
start_tailscale() {
    ...
    command -v tailscaled >/dev/null 2>&1 || {
        log_json WARN "start_tailscale" "not installed"; return 0
    }
    ...
    [ -z "${TAILSCALE_AUTHKEY:-}" ] && {
        log_json WARN "start_tailscale" "no authkey"; return 0
    }
...
check_openvpn_routing() {
    ...
    local out dev
    out=$(ip route get "$ROUTE_TEST_IP" 2>/dev/null || echo "")
...
cleanup_processes() {
    ...
    # Attendre avec gestion d'erreur (set -e)
    local pids_to_wait=""
    for pid in "$vpn_pid" "$privoxy_pid" "$nginx_pid" "$dnsmasq_pid" "$tailscaled_pid" "$unbound_pid"; do
...
reset_pids() {
    ...
    DOT_RESOLVED_IPS=""
    unset DOT_HOST_IP_MAP; declare -gA DOT_HOST_IP_MAP
}
    ...
    while true; do
        ...
        while true; do
            ...
            if [ -f /etc/privoxy/privoxy.config ]; then
                local addr
                addr=$(awk '/^[[:space:]]*listen-address/{print $2; exit}' \
                    /etc/privoxy/privoxy.config 2>/dev/null || echo "")
                [ -n "$addr" ] && proxy_port=$(echo "$addr" | awk -F: '{print $NF}')
            fi
